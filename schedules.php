<?php
session_start();
include 'db_connection.php'; // Ensure this file connects to 'ics_database'
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['search'])) {
    ...
    // Database query based on search type
    if ($searchType === 'section') {
        $query = "SELECT * FROM instructor WHERE yearandsection LIKE ? AND semester = ?";
    } elseif ($searchType === 'room') {
        $query = "SELECT * FROM instructor WHERE roomNumber LIKE ? AND semester = ?";
    }

    $stmt = $conn->prepare($query);
    $searchValueLike = "%" . $searchValue . "%";
    $stmt->bind_param("ss", $searchValueLike, $semester);
    $stmt->execute();
    ...
    if (!empty($rows)) {
        ...
        $message = 'No data found for the provided search criteria and semester.';
    }
}
?>
...
<head>
  ...
  <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Rethink+Sans:wght@800&display=swap">
  <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Medium:wght@400&display=swap">
  ...
  <script src="https://cdn.jsdelivr.net/npm/@ionic/core@5.8.6/dist/ionic/ionic.js"></script>
  <style>
    body {
...
<header class="header">
  <nav class="nav">
    <ul>
      ...
      <li class="toggle-snowflakes"><button id="toggleSnowflakes">Off</button></li>
    </ul>
  </nav>
</header>
    ...
    <form method="POST" action="">
        <div class="search-box">
            <div class="form-value">
                <h1 style="font-weight:600;">Search by</h1>
                <div class="inputbox">
                ...
                <div class="inputbox">
                    ...
                    <input type="search" name="searchValue" placeholder="Enter Section or Room Number" required/>
                </div>
                <div class="inputbox">
                ...
                <button type="submit" name="search" class="button-29" role="button">View</button>
            </div>
        </div>
    </form>
</div>
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title text-danger" id="searchResultsModalLabel">Search Results</h4>
            </div>
            <div class="modal-body">
                <?php if ($searchPerformed && !empty($rows)) : ?>
                        ...
                        <tbody>
                            <?php foreach ($rows as $row) : ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($row['yearandsection']) ?></td>
...
<div class="modal fade" id="errorMessageModal" tabindex="-1" role="dialog" aria-labelledby="errorMessageModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-danger" id="errorMessageModalLabel">Error</h5>
...
<div class="row">
  ...
  c5.8,0.7,11.6,1.3,17.4,2c-19.1,22.7-36.3,27-36.3,27l104,12.2c1.3,0.2,2.7,0.3,4,0.5C1622.7,194.2,1607,186,1593.7,159.5z"></path>
            <path class="" style="fill: rgb(0, 26, 7);" d="M1475.4,177c0,0-16.4-8.1-30.8-35.3c6,0.6,12.1,1.2,18.1,1.8
